...
function updateItemsList(eFilter, isChecked) {
    ...
    $.post(currentUrl+eFilter+"/"+(+isChecked), {}, data => {
        ...
        if ($(data).find('#itemsList')[0].innerHTML.length === 37) {
            $('#itemsList')[0].innerHTML = '<td colspan="5" class="col-12 alert alert-info text-center"><?= lang('stock_lang.entity_has_no_user'); ?></td>';
        } else {
